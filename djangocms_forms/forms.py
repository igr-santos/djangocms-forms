...
from .fields import (
    ...
    HoneyPotField,
    MultipleChoiceAutoCompleteField,
    ReCaptchaField,
)
...
class FormFieldInlineForm(forms.ModelForm):
    def clean(self):
        ...
        if cleaned_data.get(
            "field_type"
        ) in requires_choice_values and not cleaned_data.get("choice_values"):
...
class FormDefinitionAdminForm(forms.ModelForm):
    ...
    #             [
    #                 _(
    #                     "You must specify either a page or external redirect when "
...
class FormBuilder(forms.Form):
    ...
    def __init__(self, form_definition, *args, **kwargs):
        ...
        if form_definition.use_honeypot:
            ...
            field_name = "recaptcha_%s" % int_to_hashid(
                form_definition.pk, min_length=8
            )
    ...
    def prepare_select(self, field):
        ...
        if field.choice_values:
            ...
            if not field.required:
                choice_list.insert(
                    0, ("", field.placeholder_text or _("Please select an option"))
                )
    ...
    def email_submission(self, form_data, request, referrer):
        ...
        mail_subject = (
            self.form_definition.email_subject
            or "Form Submission - %s" % self.form_definition.name
        )
        context = {
        ...
        message_html = render_to_string(
            "djangocms_forms/email_template/email.html", context
        )
...
class SubmissionExportForm(forms.Form):
    ...
    file_type = forms.ChoiceField(
        choices=FORMAT_CHOICES, initial="xlsx", required=False
    )
