...
class FormDefinition(CMSPlugin):
    name = models.CharField(_("Nome do Formulário"), max_length=255)
    ...
    submit_btn_txt = models.CharField(
        ...
        max_length=100,
        default=_("Enviar"),
        help_text=_("Texto para o botão Enviar. O padrão é 'Enviar'"),
    )

    post_submit_msg = models.TextField(
        _("Mensagem de sucesso"),
        blank=True,
        default=_("Obrigado"),
        help_text=_(
            "Exiba esta mensagem aos usuários após eles enviarem seu formulário."
        ),
    )
    ...
    external_redirect = models.URLField(
        _("External URL"), blank=True, help_text=_("e.g. http://example.com/thank-you")
    )
    ...
    email_from = models.EmailField(
        _("Sender Email Address"), max_length=255, blank=True
    )
    ...
    class Meta:
        verbose_name_plural = _("formulários")
...
class FormField(models.Model):
    ...
    field_name = models.CharField(_("Nome do campo"), max_length=255, blank=True)
    placeholder_text = models.CharField(_("Placeholder"), blank=True, max_length=100)
    required = models.BooleanField(_("Obrigatório"), default=True)
    help_text = models.TextField(
        _("Descrição / Texto de ajuda"),
        blank=True,
        help_text=_("Uma descrição/instruções para este campo."),
    )
    initial = models.CharField(_("Valor padrão"), max_length=255, blank=True)
    choice_values = models.TextField(
        _("Opções"),
        blank=True,
        help_text=_(
            'Insira uma opção por linha. Para "Upload de arquivo" '
            "como tipo do campo, insira o tipo de arquivo permitido "
            "(por exemplo, .pdf), um por linha."
        ),
    )
    position = models.PositiveIntegerField(_("Posição"), blank=True, null=True)
...
class FormSubmission(models.Model):
    ...
    creation_date = models.DateTimeField(_("Data de criação"), auto_now=True)
    ...
    form_data = JSONField(_("Dados submetidos"))

    class Meta:
        verbose_name_plural = _("submissões de formulário")
